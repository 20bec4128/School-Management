...
public interface AdmitCardSettingRepository extends JpaRepository<AdmitCardSetting, Long> {

    @Query(value = """
            SELECT s.*
            FROM admit_card_settings s
            ...
            WHERE (:headOfficeId IS NULL OR s.head_office_id = :headOfficeId)
              ...
              AND (
                    ...
                    OR LOWER(COALESCE(s.admit_title_color, '')) LIKE LOWER(CONCAT('%', :search, '%'))
                    OR LOWER(COALESCE(s.exam_title_color, '')) LIKE LOWER(CONCAT('%', :search, '%'))
                    OR LOWER(COALESCE(s.subject_color, '')) LIKE LOWER(CONCAT('%', :search, '%'))
            ...
            """,
            nativeQuery = true)
    Page<AdmitCardSetting> searchAdmitCardSettings(
