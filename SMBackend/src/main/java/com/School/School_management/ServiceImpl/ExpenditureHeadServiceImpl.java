...
public class ExpenditureHeadServiceImpl implements ExpenditureHeadService {
    ...
    private final ExpenditureHeadRepository repository;
    private final SchoolRepository schoolRepository;

    public ExpenditureHeadServiceImpl(ExpenditureHeadRepository repository, SchoolRepository schoolRepository) {
    ...
    public List<ExpenditureHeadDto> list(Long schoolId) {
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<ExpenditureHeadDto> listPaginated(Long schoolId, int page, int size, String search) {
        CurrentUser user = CurrentUserHolder.get();
        if (user == null) throw new ForbiddenException();
    ...
    @Override
    public ExpenditureHeadDto create(ExpenditureHeadDto dto) {
        CurrentUser user = CurrentUserHolder.get();
        if (user == null) throw new ForbiddenException();
        Long effectiveSchoolId = effectiveSchoolIdForWrite(user, dto == null ? null : dto.getSchoolId());
        ExpenditureHead entity = new ExpenditureHead();
        entity.setSchool(resolveSchool(effectiveSchoolId));
        entity.setExpenditureHead(normalizeRequired(dto == null ? null : dto.getExpenditureHead(), "Expenditure head is required"));
        entity.setNote(normalizeOptional(dto == null ? null : dto.getNote()));
        return toDto(repository.save(entity));
    }

    @Override
    public ExpenditureHeadDto update(Long id, ExpenditureHeadDto dto) {
        CurrentUser user = CurrentUserHolder.get();
        if (user == null) throw new ForbiddenException();
        ExpenditureHead entity = repository.findById(id).orElseThrow(NotFoundException::new);
        Long effectiveSchoolId = effectiveSchoolIdForWrite(user, dto == null ? null : dto.getSchoolId());
    ...
    private ExpenditureHeadDto toDto(ExpenditureHead entity) {
        ExpenditureHeadDto dto = new ExpenditureHeadDto();
        ...
        dto.setExpenditureHead(entity.getExpenditureHead());
