...
public class AssignmentServiceImpl implements AssignmentService {
    ...
    @Override
    public List<Assignment> getAllAssignments(Long requestedSchoolId) {
        CurrentUser user = CurrentUserHolder.get();
        if (user == null) throw new ForbiddenException();

        Long schoolId = schoolGuard.schoolIdForRead(user, requestedSchoolId);

        if (user.isSuperAdmin()) {
            if (schoolId == null) return assignmentRepository.findAll();
            return assignmentRepository.findBySchoolId(schoolId);
        }

        if (user.isHeadOfficeScopedAdmin()) {
            if (schoolId != null) return assignmentRepository.findBySchoolId(schoolId);
            List<Long> schoolIds = schoolRepository.findAllByIsDeletedFalseAndHeadOfficeId(user.headOfficeId()).stream()
                    .map(s -> s.getId())
                    .filter(id -> id != null)
                    .toList();
            if (schoolIds.isEmpty()) return List.of();
            return assignmentRepository.findBySchoolIdIn(schoolIds);
        }
        ...
        if (user.isRole("TEACHER")) {
            ...
            if (schoolId != null) return assignmentRepository.findBySchoolIdAndSubjectIdIn(schoolId, subjectIds);
        ...
        if (user.isRole("STUDENT")) {
            ...
            if (studentSchoolId == null || s.getSchoolClass() == null || s.getSchoolSection() == null) return List.of();
        ...
        if (user.isRole("PARENT")) {
            ...
            for (Long sid : parentStudentRepository.findStudentIdsByParentId(user.parentId())) {
                studentRepository.findById(sid).ifPresent((s) -> {
                    Long childSchoolId = s.getSchool() == null ? null : s.getSchool().getId();
                    ...
                    if (childSchoolId == null || classId == null || sectionId == null) return;
                    assignmentRepository.findBySchoolIdAndClassIdAndSectionId(childSchoolId, classId, sectionId)
