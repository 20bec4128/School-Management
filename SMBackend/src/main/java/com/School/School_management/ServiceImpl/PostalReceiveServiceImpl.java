...
public class PostalReceiveServiceImpl implements PostalReceiveService {
    ...
    @Override
    public Page<PostalReceiveDto> pageBySchool(Long schoolId, String search, int page, int size) {
        Pageable pageable = PageRequest.of(Math.max(0, page), Math.max(1, size));
        final String q = normalize(search);
        List<PostalReceiveDto> filtered = repository.findBySchoolIdAndIsDeletedFalse(schoolId).stream()
                .map(this::toDto)
                .filter(dto -> matches(dto, q))
                .sorted(Comparator.comparing(PostalReceiveDto::getDate, Comparator.nullsLast(Comparator.reverseOrder()))
                        .thenComparing(PostalReceiveDto::getId, Comparator.nullsLast(Comparator.reverseOrder())))
                .collect(Collectors.toList());
    ...
    private boolean matches(PostalReceiveDto dto, String search) {
        if (search == null) {
            return true;
        }
        String q = search.toLowerCase(Locale.ROOT);
        return containsIgnoreCase(dto.getFromTitle(), q)
                || containsIgnoreCase(dto.getToTitle(), q)
