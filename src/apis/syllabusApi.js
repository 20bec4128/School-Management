...
export const fetchSyllabuses = async () => {
  const res = await fetch(SYLLABUSES_API_BASE, { headers: { Accept: 'application/json' } })
  if (!res.ok) throw new Error(await readApiError(res))
  return res.json()
}

export const createSyllabus = async (payload, file) => {
  const res = await fetch(SYLLABUSES_API_BASE, {
    method: 'POST',
...
export const updateSyllabus = async (id, payload, file) => {
  const res = await fetch(`${SYLLABUSES_API_BASE}/${id}`, {
    method: 'PUT',
    body: buildUpsertFormData(payload, file),
  })
  if (!res.ok) throw new Error(await readApiError(res))
...
export const deleteSyllabus = async (id) => {
  const res = await fetch(`${SYLLABUSES_API_BASE}/${id}`, { method: 'DELETE' })
  if (!res.ok) throw new Error(await readApiError(res))
  return res.text()
}
