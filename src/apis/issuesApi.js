...
const LOOKUP_BASE = '/api/issues/lookups'
...
export const fetchIssuesPage = async ({ headOfficeId, schoolId, userType, search = '', page = 0, size = 10 } = {}) => {
  ...
  if (userType != null && String(userType).trim() !== '') qs.set('userType', String(userType).trim())
...
export const fetchIssueRoles = async ({ schoolId } = {}) => {
  const qs = new URLSearchParams()
  if (schoolId != null && String(schoolId).trim() !== '') qs.set('schoolId', String(schoolId))
...
export const fetchIssueRecipients = async ({ schoolId, role } = {}) => {
  const qs = new URLSearchParams()
  if (schoolId != null && String(schoolId).trim() !== '') qs.set('schoolId', String(schoolId))
