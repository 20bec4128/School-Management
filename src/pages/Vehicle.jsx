...
const STEPS = ['Vehicle Information'];
...
const Vehicle = ({ onNavigate }) => {
  ...
  const renderForm = () => (
    <div className="avm-grid">
      ...
      <FormField label="Vehicle Number" required full>
        <input type="text" className="avm-input" id="vehicleNumber" placeholder="Vehicle Number" value={addForm.vehicleNumber} onChange={(e) => setAddForm({...addForm, vehicleNumber: e.target.value})} />
      </FormField>

      <FormField label="Vehicle Model" full>
        <input type="text" className="avm-input" id="vehicleModel" placeholder="Vehicle Model" value={addForm.vehicleModel} onChange={(e) => setAddForm({...addForm, vehicleModel: e.target.value})} />
      </FormField>

      <FormField label="Driver">
        <input type="text" className="avm-input" id="driver" placeholder="Driver" value={addForm.driver} onChange={(e) => setAddForm({...addForm, driver: e.target.value})} />
      </FormField>

      <FormField label="Vehicle License">
        <input type="text" className="avm-input" id="vehicleLicense" placeholder="Vehicle License" value={addForm.vehicleLicense} onChange={(e) => setAddForm({...addForm, vehicleLicense: e.target.value})} />
      </FormField>

      <FormField label="Vehicle Contact" required full>
        <input type="text" className="avm-input" id="vehicleContact" placeholder="Vehicle Contact" value={addForm.vehicleContact} onChange={(e) => setAddForm({...addForm, vehicleContact: e.target.value})} />
      </FormField>
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        ...
        <button className="btn btn-primary-600 d-flex align-items-center gap-6" onClick={() => onNavigate?.('vehicle-create')}>
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map(col => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === 'vehicleNumber' ? <span className="fw-medium text-primary-light">{row[col.key]}</span> : row[col.key]}
                        </td>
                      ))}
                      <td>
      ...
      <SlideSidebar isOpen={isFilterSidebarOpen} onClose={() => setIsFilterSidebarOpen(false)} title="Find Vehicle">
