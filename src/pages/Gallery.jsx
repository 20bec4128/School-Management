...
const galleryList = [
  {
    ...
    note: 'Manage Gallery Image List Add',
    isViewOnWeb: true,
  },
  {
    sl: '02',
    ...
    note: 'Photos and memories from annual day celebration',
    isViewOnWeb: false,
  },
  {
    sl: '03',
    ...
    note: 'Outdoor activity and student participation gallery',
    isViewOnWeb: true,
  },
  {
    sl: '04',
    school: 'Windsor Park High School',
    title: 'Science Expo',
    note: 'Exhibition photos from school science fair',
    isViewOnWeb: true,
  },
  {
    sl: '05',
    ...
    note: 'Learning and classroom activities snapshot collection',
    isViewOnWeb: false,
  },
]
...
const Gallery = () => {
  ...
  const schoolOptions = useMemo(() => Array.from(new Set(galleryList.map((item) => item.school))), [])

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()

    return galleryList.filter((row) => {
      const matchesSearch =
        !q ||
        [row.school, row.title, row.note, row.isViewOnWeb ? 'yes' : 'no']
          .join(' ')
          .toLowerCase()
          .includes(q)
