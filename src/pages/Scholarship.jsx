...
const scholarships = [
  {
    ...
    note: 'Merit-based scholarship for outstanding academic performance.',
  },
  {
    sl: '02',
    school: 'Windsor Park High School',
  ...
  },
  {
    ...
    note: 'Need-based financial support.',
  },
  {
    sl: '04',
    school: 'Windsor Park High School',
...
const Scholarship = () => {
  ...
  const schoolOptions = useMemo(
    () => Array.from(new Set(scholarships.map((r) => r.school))),
    [],
  )

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    return scholarships.filter((r) => {
      const matchesSearch =
        !q ||
        [r.school, r.candidate, r.className, r.section, r.rollNo, r.amount, r.paymentDate, r.note]
          .join(' ')
          .toLowerCase()
          .includes(q)
            ...
            <table className="table bordered-table mb-0 data-table" style={{ minWidth: 900 }}>
              <thead>
                <tr>
                  ...
                  {visibleColumns.candidate ? <th scope="col">Candidate</th> : null}
                  ...
                  paginated.map((row) => (
                    <tr key={row.sl}>
                      ...
                      {visibleColumns.candidate ? (
                        <td className="fw-medium text-primary-light">{row.candidate}</td>
                      ) : null}
