...
const FeeCollectionReport = () => {
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      return matchesSearch && matchesSchool && matchesYear && matchesClass && matchesMethod
    })
  }, [data, search, filters])
  ...
  const renderCell = (row, column) => {
    ...
    if (column.key === 'paidAmount') {
      return <span className="fw-bold text-success-600">{value || '0'}</span>
    }
    ...
    <div className="dashboard-main-body">
      ...
      >
        <form className="p-20 d-grid gap-16" onSubmit={handleApplyFilters}>
          ...
          <div>
            <label className="text-sm fw-semibold text-primary-light d-inline-block mb-8">Payment Method</label>
            <select
              className="form-control form-select"
              value={pendingFilters.paymentMethod}
              onChange={(e) => setPendingFilters((p) => ({ ...p, paymentMethod: e.target.value }))}
            >
