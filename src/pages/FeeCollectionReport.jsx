  ...
  const exportRows = useMemo(
    () =>
      rows.map((row) => ({
        ...
        'Fee Type': row.feeTypeTitle || '--',
        'Invoice No.': row.invoiceNumber || '--',
        Month: row.month || '--',
        Status: row.paidStatus || '--',
        'Gross Amount': row.grossAmount != null ? Number(row.grossAmount).toFixed(2) : '--',
        Discount: row.discount != null ? Number(row.discount).toFixed(2) : '--',
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setTotalElements(0)
        setTotalPages(0)
        setLoadError(error?.message || 'Failed to load fee collection report')
      } finally {
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          <h1 className="fw-semibold mb-4 h6 text-primary-light">Fee Collection Report</h1>
          <span className="text-secondary-light">Dashboard / Fee Collection Report</span>
        </div>
      </div>
          ...
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-16 px-20 py-12 border-bottom border-neutral-200">
            <div className="d-flex flex-wrap align-items-center gap-16">
              <ExportDropdown
                ...
                sheetName="Fee Collection Report"
                pdfTitle="Fee Collection Report"
              />
                ...
                {busy && rows.length === 0 ? (
                  <tr>
                    <td colSpan={visibleColumnCount + 1} className="text-center py-40 text-secondary-light">
                      Loading fee collection report...
                    </td>
                  </tr>
                ) : rows.length === 0 ? (
                  <tr>
                    <td colSpan={visibleColumnCount + 1} className="text-center py-40 text-secondary-light">
                      {canLoadRows ? 'No fee collection records found.' : 'Select a school to view the fee collection report.'}
                    </td>
                  </tr>
                ) : (
                  rows.map((row, idx) => (
      ...
      <SlideSidebar
        ...
        title="Filter Fee Collection Report"
