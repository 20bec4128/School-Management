...
const LibraryReport = () => {
  ...
  const renderCell = (row, column) => {
    ...
    if (column.key === 'issue') {
      return <span className="text-primary-600">{value || '0'}</span>
    }
    if (column.key === 'returnCount') {
      return <span className="text-success-600">{value || '0'}</span>
    }
    return value || '--'
  }
    ...
    <div className="dashboard-main-body">
      ...
      >
        <form className="p-20 d-grid gap-16" onSubmit={handleApplyFilters}>
          ...
          <div>
            <label className="text-sm fw-semibold text-primary-light d-inline-block mb-8">Category</label>
            <select
              className="form-control form-select"
