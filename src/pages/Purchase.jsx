...
const Purchase = () => {
  ...
  const handleExportExcel = async () => {
    try {
          ...
          columnOptions.reduce(
            (acc, column) => {
              if (visibleColumns[column.key]) {
                acc[column.label] =
                  row[column.key] ??
                  (column.key === 'unitTypeLabel'
                    ? row.unitType === 'OTHER'
                      ? row.customUnitType || 'Other'
                      : row.unitType || ''
                    : column.key === 'schoolName'
                      ? row.schoolName || resolveSchoolName(row.schoolId)
                      : column.key === 'supplierName'
                        ? row.supplierName || resolveSupplierName(row.supplierId)
                        : column.key === 'categoryName'
                          ? row.categoryName || resolveCategoryName(row.categoryId)
                          : column.key === 'productName'
                            ? row.productName || resolveProductName(row.productId)
                            : column.key === 'purchaseByName'
                              ? row.purchaseByName || resolveEmployeeName(row.purchaseById)
                              : '')
              }
              return acc
            },
      ...
      console.error('Failed to export purchases:', err)
