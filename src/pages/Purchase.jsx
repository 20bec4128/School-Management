...
const columnOptions = [
  ...
  { key: 'purchaseBy', label: 'Purchase By' },
  ...
  const renderForm = () => (
    <div className="avm-grid">
      ...
      <FormField label="Supplier" required>
        <select className="avm-select" id="supplierId" value={addForm.supplierId} onChange={(e) => setAddForm({ ...addForm, supplierId: e.target.value })}>
          <option value="">--Select--</option>
        </select>
      </FormField>
      <FormField label="Category" required>
        <select className="avm-select" id="categoryId" value={addForm.categoryId} onChange={(e) => setAddForm({ ...addForm, categoryId: e.target.value })}>
          <option value="">--Select--</option>
        </select>
      </FormField>
      <FormField label="Product" required full>
        <select className="avm-select" id="productId" value={addForm.productId} onChange={(e) => setAddForm({ ...addForm, productId: e.target.value })}>
          <option value="">--Select--</option>
        </select>
      </FormField>
      <FormField label="Quantity" required>
        <input type="number" className="avm-input" id="quantity" placeholder="Quantity" value={addForm.quantity} onChange={(e) => setAddForm({ ...addForm, quantity: e.target.value })} />
      </FormField>
      <FormField label="Unit Type">
        <select className="avm-select" id="unitType" value={addForm.unitType} onChange={(e) => setAddForm({ ...addForm, unitType: e.target.value })}>
          <option value="">--Select--</option>
        </select>
      </FormField>
      <FormField label="Unit Price" required>
        <input type="number" className="avm-input" id="unitPrice" placeholder="Unit Price" value={addForm.unitPrice} onChange={(e) => setAddForm({ ...addForm, unitPrice: e.target.value })} />
      </FormField>
      <FormField label="Purchase Date" required>
        <input type="date" className="avm-input" id="purchaseDate" value={addForm.purchaseDate} onChange={(e) => setAddForm({ ...addForm, purchaseDate: e.target.value })} />
      </FormField>
      <FormField label="Expire Date" required>
        <input type="date" className="avm-input" id="expireDate" value={addForm.expireDate} onChange={(e) => setAddForm({ ...addForm, expireDate: e.target.value })} />
      </FormField>
      <FormField label="Purchase By" required full>
        <select className="avm-select" id="purchaseBy" value={addForm.purchaseBy} onChange={(e) => setAddForm({ ...addForm, purchaseBy: e.target.value })}>
          <option value="">--Select--</option>
        </select>
      </FormField>
        ...
        <div className="card-body p-0 dataTable-wrapper">
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-16 px-20 py-12 border-bottom border-neutral-200">
            <div className="d-flex flex-wrap align-items-center gap-16">
              <div className="dropdown">
                ...
                <ul className="dropdown-menu p-12 border bg-base shadow">
                  <li><button type="button" className="dropdown-item px-16 py-8 rounded text-secondary-light bg-hover-neutral-200 d-flex align-items-center gap-10" onClick={handleExportExcel}><i className="ri-file-text-line"></i> CSV</button></li>
            ...
            <div className="position-relative">
              <input type="text" className="form-control ps-40 py-9 border border-neutral-300 radius-8 text-secondary-light" placeholder="Search purchases..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map((col) => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === 'totalPrice' ? <span className="fw-medium text-primary-light">{row[col.key]}</span> : row[col.key]}
                        </td>
                      ))}
                      <td>
                        <div className="d-flex align-items-center gap-10">
                          <button type="button" className="bg-info-focus bg-hover-info-200 text-info-600 w-32-px h-32-px d-flex align-items-center justify-content-center rounded-circle"><i className="ri-edit-line"></i></button>
                          <button type="button" className="bg-danger-focus bg-hover-danger-200 text-danger-600 w-32-px h-32-px d-flex align-items-center justify-content-center rounded-circle"><i className="ri-delete-bin-line"></i></button>
                        </div>
                      </td>
                    </tr>
                  ))
