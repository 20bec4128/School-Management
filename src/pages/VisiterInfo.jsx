    ...
    try {
      if (scopedSchoolIds.length === 1) {
        const [infoList, purposeList] = await Promise.all([
          ...
          fetchVisitorPurposes(scopedSchoolIds[0]),
        ]);
        ...
        setPurposes(
          uniqueBy(
            Array.isArray(purposeList) ? purposeList : [],
            (row) => row.id,
          ),
        );
        return;
      ...
      const [infoRows, purposeRows] = await Promise.all([
        fetchRowsForSchoolIds(scopedSchoolIds, async (schoolId) => {
          const response = await fetchVisitorInfosPage({
            schoolId,
            search,
            page: 0,
            size: 1000,
        ...
        fetchRowsForSchoolIds(scopedSchoolIds, fetchVisitorPurposes),
      ]);

      const mergedRows = uniqueBy(Array.isArray(infoRows) ? infoRows : [], (row) => row.id)
      ...
      setPurposes(uniqueBy(Array.isArray(purposeRows) ? purposeRows : [], (row) => row.id));
    } catch (err) {
      ...
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        ...
        <div className="d-flex flex-wrap align-items-center gap-12">
          <button
            ...
            title={
              !isSuperAdmin && !listSchoolId ? "Select a school first" : ""
            }
          >
            ...
            >
              <thead>
                <tr>
                  ...
                  {visibleColumns.purposeName ? (
                    <th scope="col">Visitor Purpose</th>
                  ) : null}
          ...
          {isSuperAdmin ? (
            <div className="full">
              <ManualScopeSelectors
                ...
                onSchoolChange={(val) =>
                  setPendingFilters((p) => ({ ...p, schoolId: val }))
                }
              />
            </div>
          ) : (
