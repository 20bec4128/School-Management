    ...
    const load = async () => {
      ...
      try {
        const data = await fetchAllPages(
          fetchExpendituresPage,
          {
            schoolId: selectedSchoolId || null,
            expenditureHeadId:
              filters.expenditureHeadId && filters.expenditureHeadId !== 'Select'
                ? filters.expenditureHeadId
                : null,
            expenditureMethod:
              filters.expenditureMethod && filters.expenditureMethod !== 'Select'
                ? filters.expenditureMethod
                : null,
            startDate: filters.startDate || null,
            endDate: filters.endDate || null,
        ...
        if (!cancelled) setGraphRows(Array.isArray(data) ? data : [])
      } catch {
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          <h1 className="fw-semibold mb-4 h6 text-primary-light">Expenditure Report</h1>
          <span className="text-secondary-light">Dashboard / Expenditure Report</span>
        </div>
      </div>
          ...
          <ReportTabPanel active={activeTab === 'graph'}>
            {graphBusy && graphRows.length === 0 ? (
              ...
              <ReportGraphPanel
                ...
                emptyMessage={
                  canLoadRows
                    ? 'No expenditure records found for chart.'
                    : 'Select a school to view the graphical expenditure report.'
                }
                hint="category"
                labelKeys={['expenditureHeadName', 'expenditureMethod', 'reference', 'schoolName', 'expenditureDate']}
                valueKey="amount"
              />
            )}
          </ReportTabPanel>
      ...
      <SlideSidebar
        ...
        title="Filter Expenditure Report"
