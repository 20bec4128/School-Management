...
const ManageTodo = ({ onNavigate }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      const matchesSearch = !q || row.title?.toLowerCase().includes(q) || row.assignTo?.toLowerCase().includes(q);
    ...
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        ...
        <button 
          ...
          onClick={() => onNavigate?.("manage-todo-create")}
        >
          <i className="ri-add-large-line"></i> Add Todo
        </button>
      </div>
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map((col) => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === "workStatus" ? (
                            <span className={workStatusBadge(row[col.key])}>{row[col.key]}</span>
                          ) : col.key === "title" ? (
