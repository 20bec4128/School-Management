...
const postalDispatches = [
  { sl: '01', school: 'Windsor Park High School', toTitle: 'Ministry of Education', reference: 'REF-2024-001', fromTitle: 'Principal', dispatchDate: '2024-03-15' },
  { sl: '02', school: 'Windsor Park High School', toTitle: 'District Office', reference: 'REF-2024-002', fromTitle: 'Vice Principal', dispatchDate: '2024-03-16' },
  { sl: '03', school: 'Windsor Park High School', toTitle: 'Parents Association', reference: '', fromTitle: 'Head Teacher', dispatchDate: '2024-03-17' },
  { sl: '04', school: 'Windsor Park High School', toTitle: 'Local Authority', reference: 'REF-2024-004', fromTitle: 'Principal', dispatchDate: '2024-03-18' },
  { sl: '05', school: 'Windsor Park High School', toTitle: 'Board of Governors', reference: 'REF-2024-005', fromTitle: 'Vice Principal', dispatchDate: '2024-03-19' },
]
...
const PostalDispatch = () => {
  ...
  const schoolOptions = useMemo(
    () => Array.from(new Set(postalDispatches.map((r) => r.school))),
    [],
  )

  const filtered = useMemo(() => {
    const q = search.trim().toLowerCase()
    return postalDispatches.filter((r) => {
      const matchesSearch = !q || [r.school, r.toTitle, r.reference, r.fromTitle, r.dispatchDate].join(' ').toLowerCase().includes(q)
            ...
            <table className="table bordered-table mb-0 data-table" style={{ minWidth: 800 }}>
              <thead>
                <tr>
                  ...
                  {visibleColumns.dispatchDate ? <th scope="col">Dispatch Date</th> : null}
                  <th scope="col">Action</th>
                </tr>
              </thead>
              <tbody>
                {paginated.length === 0 ? (
                  <tr><td colSpan={visibleColumnCount} className="text-center py-40 text-secondary-light">No postal dispatches found.</td></tr>
                ) : paginated.map((row) => (
                  <tr key={row.sl}>
                    <td>
                        <div className="form-check style-check d-flex align-items-center">
                    ...
                    {visibleColumns.dispatchDate ? <td>{row.dispatchDate}</td> : null}
                    <td>
