...
const InvoiceReport = () => {
  ...
  const [pendingFilters, setPendingFilters] = useState(() =>
    makeDefaultFilters(initialSchoolId),
  );
  ...
  const feeTypeOptions = useMemo(
    () => (Array.isArray(feeTypes) ? feeTypes : []),
    [feeTypes],
  );
  ...
  }, [status, token, pendingFilters.schoolId, isSchoolAdmin, authSchoolId]);
    ...
    const load = async () => {
      ...
      try {
        const data = await fetchFeeCollectionsPage({
          ...
          classId:
            filters.classId && filters.classId !== "Select"
              ? filters.classId
              : null,
          feeTypeId:
            filters.feeTypeId && filters.feeTypeId !== "Select"
              ? filters.feeTypeId
              : null,
          status:
            filters.status && filters.status !== "Select"
              ...
              : null,
          month:
            filters.month && filters.month !== "Select" ? filters.month : null,
    ...
    const data = await fetchFeeCollectionsPage({
      ...
      feeTypeId:
        filters.feeTypeId && filters.feeTypeId !== "Select"
          ...
          : null,
      status:
        filters.status && filters.status !== "Select" ? filters.status : null,
                  ...
                  <tr>
                    ...
                    >
                      {canLoadRows
                        ...
                        : "Select a school to view the invoice report."}
                    </td>
                  </tr>
                ) : (
                  rows.map((row, idx) => (
