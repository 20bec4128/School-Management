...
const SMS = ({ onNavigate }) => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      const matchesSearch = !q || row.sms?.toLowerCase().includes(q);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "SMSHistory");
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          <h1 className="fw-semibold mb-4 h6 text-primary-light">SMS</h1>
          <span className="text-secondary-light">Communications / SMS</span>
        </div>
        <button 
          className="btn btn-primary-600 d-flex align-items-center gap-6" 
          onClick={() => onNavigate?.("sms-create")}
        >
          <i className="ri-chat-1-line"></i> Send SMS
        </button>
      </div>
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map((col) => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === "sms" ? <span className="text-wrap" style={{maxWidth: '300px', display: 'block'}}>{row[col.key]}</span> : row[col.key]}
                        </td>
                      ))}
                      <td>
      ...
      <SlideSidebar isOpen={isFilterOpen} onClose={() => setIsFilterOpen(false)} title="Find SMS">
