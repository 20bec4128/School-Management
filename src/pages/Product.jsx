...
const Product = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((row) => {
      ...
      const matchesCategory = filters.category === 'Select' || row.category === filters.category;
        ...
        <div className="card-body p-0 dataTable-wrapper">
          ...
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-16 px-20 py-12 border-bottom border-neutral-200">
            ...
            <div className="position-relative">
              <input type="text" className="form-control ps-40 py-9 border border-neutral-300 radius-8 text-secondary-light" placeholder="Search products..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map(col => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === 'productName' ? <span className="fw-medium text-primary-light">{row[col.key]}</span> : row[col.key]}
                        </td>
                      ))}
                      <td>
      ...
      <SlideSidebar isOpen={isFilterSidebarOpen} onClose={() => setIsFilterSidebarOpen(false)} title="Find Product">
        <form className="p-20 d-grid gap-16" onSubmit={(e) => { e.preventDefault(); setIsFilterSidebarOpen(false); }}>
          ...
          <FormField label="Category">
            <select className="avm-select" value={filters.category} onChange={(e) => setFilters({ ...filters, category: e.target.value })}>
