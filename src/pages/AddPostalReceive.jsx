...
import { createPostalReceive, updatePostalReceive } from '../apis/postalApi'
...
const readEditRow = () => {
  try {
    const raw = sessionStorage.getItem('edit-postal-receive-row')
...
const AddPostalReceive = ({ onNavigate }) => {
  ...
  useEffect(() => () => sessionStorage.removeItem('edit-postal-receive-row'), [])
  ...
  useEffect(() => {
    if (!isSuperAdmin || !initialEditRow || schools.length === 0) return
  ...
  }, [schools, initialEditRow, isSuperAdmin, manualScope])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await updatePostalReceive(editingId, payload)
      } else {
        await createPostalReceive(payload)
      }
      setSuccess(true)
      setTimeout(() => onNavigate('postal-receive'), 1000)
    } catch (err) {
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex align-items-center justify-content-between mb-24">
        <div>
          <h1 className="h6 text-primary-light">{editingId ? 'Edit' : 'Add'} Postal Receive</h1>
          <span className="text-secondary-light">Postal Receive / {editingId ? 'Edit' : 'Add'}</span>
        </div>
        <button className="btn btn-light border" onClick={() => onNavigate('postal-receive')}>Back to List</button>
      </div>
      ...
      <div className="card h-100">
        <form className="card-body p-24" onSubmit={handleSubmit}>
          <div className="avm-grid">
            ...
            <div className="avm-field"><label className="avm-label">Receive Date <span className="req">*</span></label>
...
export default AddPostalReceive
