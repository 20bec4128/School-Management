...
const feeTypesData = [
  {
    ...
    feeType: 'General Fee',
    title: 'Tuition Fee',
    note: 'Monthly tuition fee for regular classes',
  },
  {
    sl: '02',
    school: 'Windsor Park High School',
    ...
    note: 'Yearly hostel accommodation charges',
  },
  {
    sl: '03',
    school: 'Windsor Park High School',
    feeType: 'Transport',
    title: 'Bus Fare',
    note: 'Monthly transportation fee',
  },
  {
    sl: '04',
    school: 'Windsor Park High School',
    feeType: 'Product Sale',
    title: 'Uniform Fee',
    note: 'One-time uniform purchase fee',
  },
  {
    sl: '05',
    school: 'Windsor Park High School',
    feeType: 'General Fee',
    title: 'Library Fee',
    note: 'Annual library membership fee',
  },
]

const feeTypeOptions = ['General Fee', 'Product Sale', 'Hostel', 'Transport']
...
const FeeType = () => {
  ...
  const schoolOptions = useMemo(() => Array.from(new Set(feeTypesData.map((item) => item.school))), [])

  const filteredData = useMemo(() => {
    const q = search.trim().toLowerCase()
    return feeTypesData.filter((row) => {
      const matchesSearch = !q || [row.school, row.title, row.note].join(' ').toLowerCase().includes(q)
      const matchesSchool = filters.school === 'All' || row.school === filters.school
      const matchesFeeType = filters.feeType === 'All' || row.feeType === filters.feeType

      return matchesSearch && matchesSchool && matchesFeeType
    })
  }, [search, filters])
        ...
        <>
          ...
          <div className="avm-grid">
            ...
            <FormField label="Fee Type" required full>
              <select className="avm-select" id="feeType" value={form.feeType} onChange={handleChange(setter)}>
            ...
            <FormField label="Note" full noIcon>
              <textarea rows="3" className="avm-input avm-textarea" id="note" placeholder="Enter note" value={form.note} onChange={handleChange(setter)} />
            </FormField>
          </div>
        </>
      ) : null}
  ...
  return (
    <div className="dashboard-main-body">
      ...
      <SlideSidebar isOpen={isFilterSidebarOpen} onClose={() => setIsFilterSidebarOpen(false)} title="Filter Fee Type">
        <form className="p-20 d-grid grid-cols-2 gap-16" onSubmit={handleApplyFilters}>
          ...
          <div style={{ gridColumn: '1 / -1' }}>
            <label htmlFor="feeType" className="text-sm fw-semibold text-primary-light d-inline-block mb-8">Fee Type</label>
            <select id="feeType" className="form-control form-select" value={pendingFilters.feeType} onChange={handlePendingFilterChange}>
