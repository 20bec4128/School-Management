...
const columnOptions = [
  ...
  ...months.map(m => ({ key: m.toLowerCase(), label: m })),
  { key: 'totalPercentage', label: 'Total %' },
];

const EmployeeYearlyAttendanceReport = () => {
  ...
  // Logic from TeacherYearlyAttendanceReport reference
  const renderPercentage = (value) => {
    const val = parseFloat(value) || 0;
    let colorClass = 'text-danger-600';
    if (val >= 95) colorClass = 'text-success-600';
    else if (val >= 85) colorClass = 'text-primary-600';
    else if (val >= 70) colorClass = 'text-warning-600';

    return <span className={`fw-bold ${colorClass}`}>{val}%</span>;
  };
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          <h1 className="fw-semibold mb-4 h6 text-primary-light">Employee Yearly Attendance Report</h1>
          <span className="text-secondary-light">Dashboard / Employee Yearly Attendance</span>
        </div>
      </div>
          ...
          <div className="p-0 table-responsive">
            <table className="table bordered-table mb-0 data-table" style={{ minWidth: 1200 }}>
              <thead>
                <tr>
                  ...
                  {columnOptions.map((col) => visibleColumns[col.key] && (
                    <th scope="col" key={col.key} className={col.key === 'employeeName' ? '' : 'text-center'}>
                      {col.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={visibleColumnCount + 1} className="text-center py-40 text-secondary-light">No employee records found.</td>
                  </tr>
                ) : (
                  paginated.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map((col) => visibleColumns[col.key] && (
                        <td key={col.key} className={col.key === 'employeeName' ? 'fw-medium text-primary-light' : 'text-center'}>
                          {col.key === 'totalPercentage' ? renderPercentage(row[col.key]) : (row[col.key] || '0%')}
                        </td>
                      ))}
                    </tr>
                  ))
      ...
      <SlideSidebar isOpen={isFilterSidebarOpen} title="Filter Yearly Attendance" onClose={() => setIsFilterSidebarOpen(false)}>
...
export default EmployeeYearlyAttendanceReport;