...
const AssetItem = ({ onNavigate }) => {
  ...
  const handleExportExcel = () => {
    ...
    XLSX.writeFile(workbook, "Item_Inventory.xlsx");
  };

  const handleExportPDF = () => {
    const doc = new jsPDF({ orientation: 'landscape' });
    doc.text("Asset Item Inventory Report", 14, 10);
    doc.autoTable({
  ...
  return (
    <div className="dashboard-main-body">
      <div className="breadcrumb d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <div>
          <h1 className="fw-semibold mb-4 h6 text-primary-light">Item</h1>
          <span className="text-secondary-light">Inventory / Item Management</span>
        </div>
        <button className="btn btn-primary-600 d-flex align-items-center gap-6" onClick={() => onNavigate?.('add-asset-item')}>
          <i className="ri-add-large-line"></i> Add Asset Item
        </button>
      </div>
        ...
        <div className="card-body p-0 dataTable-wrapper">
          ...
          <div className="d-flex align-items-center justify-content-between flex-wrap gap-16 px-20 py-12 border-bottom border-neutral-200">
            ...
            <div className="position-relative">
              <input type="text" className="form-control ps-40 py-9 border border-neutral-300 radius-8 text-secondary-light" placeholder="Search items..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} />
                  ...
                  paginatedData.map((row, idx) => (
                    <tr key={idx}>
                      ...
                      {columnOptions.map(col => visibleColumns[col.key] && (
                        <td key={col.key}>
                          {col.key === 'status' ? (
                            <span className="px-12 py-4 radius-4 fw-medium text-sm bg-success-100 text-success-600">Available</span>
                          ) : col.key === 'itemName' ? (
      ...
      <SlideSidebar isOpen={isFilterSidebarOpen} onClose={() => setIsFilterSidebarOpen(false)} title="Find Asset Item">
